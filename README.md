...
- **投画面（整屏镜像）**：两条路 —— ① **应用内投屏（自研）**：录屏 H.264 → 自研 MPEG-TS 封包 → DLNA 实时流推送，坚果 N1S 4K 等 DLNA 投影/电视可用（延迟约 1~3 秒）；② 调起**系统无线投屏（Miracast）**，投到 Miracast 接收端。
...
| 系统无线投屏（Miracast） | 电视 / 投影等 Miracast 接收端 | 整屏画面镜像（由系统驱动，App 仅一键调起） |
